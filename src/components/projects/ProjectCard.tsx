    ...
    <Card className="group glass-card gradient-border-hover tilt-card">
      ...
      <CardHeader>
        <div className="flex items-center justify-between mb-2">
          <CardTitle className="text-xl font-semibold text-slate-800">{project.title}</CardTitle>
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-2">
            {project.technologies.map((tech, idx) => (
              <span key={idx} className="px-3 py-1 bg-blue-100 text-blue-900 text-sm rounded-full font-medium">{tech}</span>
            ))}
          </div>
        </div>
